...
for t in xrange(100000):
    ...
    l3 = sigma(np.dot(l2, w2))
    l4 = sigma(np.dot(l3, w3))
    
    #error + change calc
    l4_error = y - l4    
    l4_change = l4_error*sigma_deriv(l4)
    l3_error = l4_change.dot(w3.T)
    l3_change = l3_error * sigma_deriv(l3)
...
print sigma(np.dot(sigma(np.dot(sigma(np.dot(sigma(np.dot(C, w0)), w1)), w2)), w3))